...
function normalizeStats(data){
  ...
  if(Array.isArray(data.geo?.rows)) data.geo.rows=data.geo.rows.map(r=>({...r,population:Math.max(0,Number(r.population)||0),ishsizlar:Math.max(0,Math.min(Number(r.ishsizlar)||0,Number(r.population)||0))}));
  return data;
}
...
app.get('/api/health',(_req,res)=>res.json({ok:true,service:'surxondaryo-live-dashboard',appsScriptConfigured:true,appsScriptUrl:APPS_SCRIPT_URL,cacheEntries:cache.size,timestamp:new Date().toISOString()}));
...
app.get('/api/families',async(req,res)=>{try{const page=Math.max(1,Number(req.query.page||1)),pageSize=Math.min(250,Math.max(1,Number(req.query.pageSize||100)));res.set('Cache-Control','no-store').json(await fetchAppsScript('families',{page,pageSize,q:req.query.q,tuman:req.query.tuman,mahalla:req.query.mahalla,kocha:req.query.kocha}))}catch(e){console.error('families:',e.message);res.status(502).json({ok:false,degraded:true,error:e.message,page:1,pageSize:100,totalPages:1,total:0,records:[]})}});
