...
async function get(path,params={}){const u=new URL(API+path,location.origin);Object.entries(params).forEach(([k,v])=>{if(v!==''&&v!=null)u.searchParams.set(k,v)});u.searchParams.set('_',Date.now());const r=await fetch(u,{cache:'no-store'});const t=await r.text();if(!r.ok)throw Error('API HTTP '+r.status);const j=JSON.parse(t);if(j&&j.ok===false)throw Error(j.error||'API xatolik');return j}
...
function render(){const a=stats.agg||{},geo=stats.geo&&A(stats.geo.rows),cov=A(stats.coverage);$('total').textContent=Number(a.n||0).toLocaleString('uz-UZ');$('kpis').innerHTML=[['Xonadon',a.n],['Aholi',a.population],['Mehnat yoshi',a.mehnatYosh],['Ishsiz',a.ishsizlar],['Nogiron shaxs',a.nogironShaxs]].map(x=>'<div class="kpi"><span>'+x[0]+'</span><b>'+Number(x[1]||0).toLocaleString('uz-UZ')+'</b></div>').join('');
const geoMap={};geo.forEach(x=>geoMap[x.key]=x.families||0);bars('income',a.incomeDist);bars('social',{Ha:a.ijtimoiyReestr?.Ha||0,"Yo'q":a.ijtimoiyReestr?.["Yo'q"]||0,Nafaqa:a.nafaqa?.Ha||0,Nogiron:a.nogironShaxs||0});bars('utility',{Internet:a.internet?.Ha||0,'Tabiiy gaz':a.tabiiyGaz?.Ha||0,'Ichimlik suvi':a.ichimlikSuvi?.Ha||0});bars('age',a.ageBands);bars('jobs',a.employment);bars('problems',Object.fromEntries(A(a.topProblems)));bars('needs',Object.fromEntries(A(a.topNeeds)));bars('problems2',Object.fromEntries(A(a.topProblems)));bars('needs2',Object.fromEntries(A(a.topNeeds)));
